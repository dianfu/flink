...
def extract_process_function(udf_proto, ctx, on_timer_ctx, collector, keyed_state_backend):
    process_function = pickle.loads(udf_proto.payload)
    ...
    def wrapped_func(value):
        if value[0] is not None:
            # it is timer data
            # VALUE: TIMER_FLAG, TIMESTAMP_OF_TIMER, CURRENT_WATERMARK, CURRENT_KEY, None
            on_timer_ctx.set_timestamp(value[1])
            on_timer_ctx.timer_service().set_current_watermark(value[2])
            keyed_state_backend.set_current_key(value[3])
            if value[0] == 0:
            ...
            keyed_state_backend.set_current_key(Row(value[4][0]))
        ...
        for result in collector.buf:
            ...
            # 2: normal data
            # result_row: [TIMER_FLAG, TIMER_TYPE, CURRENT_KEY, RESULT_DATA]
            if result[0] == 2:
