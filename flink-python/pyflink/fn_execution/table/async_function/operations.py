...
class AsyncScalarFunctionOperation(Operation, AsyncOperationMixin):
    """
    Operation for executing Python async scalar functions in Table API.
    ...
    Note: This class implements AsyncOperationMixin to integrate with the async processing
    framework, which automatically calls set_output_processor during initialization.
    """

    def __init__(self, serialized_fn):

        # Import async components from DataStream
        from pyflink.fn_execution.datastream.process.async_function.operation import (
            AsyncFunctionRunner, Emitter, ResultHandler)
        ...
        # Extract scalar functions and user defined funcs
        scalar_functions, variable_dict, self.user_defined_funcs = reduce(
            lambda x, y: (
                ','.join([x[0], y[0]]),
                dict(chain(x[1].items(), y[1].items())),
                x[2] + y[2]),
            [operation_utils.extract_user_defined_function(udf, one_arg_optimization=False)
             for udf in serialized_fn.udfs])

        # Create the eval function
        self._eval_func = eval('lambda value: [%s]' % scalar_functions, variable_dict)

        # Async execution configuration from Java side (via proto)
        # Read from serialized_fn if available, otherwise use defaults
        if hasattr(serialized_fn,
                   'async_buffer_capacity') and serialized_fn.async_buffer_capacity > 0:
            self._capacity = serialized_fn.async_buffer_capacity
        else:
            self._capacity = 100  # Default capacity

        if hasattr(serialized_fn, 'async_timeout_ms') and serialized_fn.async_timeout_ms > 0:
            self._timeout = serialized_fn.async_timeout_ms / 1000.0  # Convert to seconds
        else:
            self._timeout = 60  # Default 60 seconds

        # Retry configuration from Java side
        if hasattr(serialized_fn, 'async_retry_enabled'):
            self._retry_enabled = serialized_fn.async_retry_enabled
        else:
            self._retry_enabled = False  # Retry disabled by default

        if hasattr(serialized_fn, 'async_max_attempts') and serialized_fn.async_max_attempts > 0:
            self._max_attempts = serialized_fn.async_max_attempts
        else:
            self._max_attempts = 3  # Default max attempts

        if hasattr(serialized_fn,
                   'async_retry_delay_ms') and serialized_fn.async_retry_delay_ms > 0:
            self._retry_delay = serialized_fn.async_retry_delay_ms / 1000.0  # Convert to seconds
        else:
            self._retry_delay = 0.1  # Default 100ms delay
    ...
    def close(self):
        """Clean up async execution infrastructure."""
        # Stop emitter
    ...
    def process_element(self, value):
        ...
        from pyflink.fn_execution.datastream.process.async_function.operation import ResultHandler

        # Extract the actual data (for Table API, value format may differ from DataStream)
        # For scalar functions, we process the input row directly
        ...
        # Submit async invocation (non-blocking)
        self._async_invoke(value, result_handler)

    def _async_invoke(self, record, result_handler):
        """
        Invoke the async scalar function with retry support if enabled.

        This submits the coroutine to the AsyncFunctionRunner's event loop
        and returns immediately without blocking.
        """
        from pyflink.fn_execution.datastream.process.async_function.operation import (
            RetryableResultHandler)
        ...
        async def execute_async():
            try:
                ...
                results = self._eval_func(record)
        ...
        # If retry is enabled, wrap with RetryableResultHandler
        if self._retry_enabled:
            ...
            retry_strategy = async_retry_strategies.FixedDelayRetryStrategy(
                ...
                result_predicate=None,  # Retry on exception only
            ...
            self._async_function_runner.run_async(
                lambda r: execute_async(),
                record,
                retryable_handler)
        else:
            # No retry, use simple handler
            # Submit to event loop asynchronously
            self._async_function_runner.run_async(
                lambda r: execute_async(),
                record,
