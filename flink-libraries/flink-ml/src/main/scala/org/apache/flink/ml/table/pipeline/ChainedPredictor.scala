...
object ChainedPredictor{
  ...
  implicit def chainedPredictOperation[
      ...
      Intermediate,
      Prediction](
      implicit transformOperation: TransformDataSetOperation[T],
      predictOperation: PredictDataSetOperation[P])
    : PredictDataSetOperation[ChainedPredictor[T, P]] = {

    new PredictDataSetOperation[ChainedPredictor[T, P]] {
  ...
  implicit def chainedFitOperation[L <: Transformer[L], R <: Predictor[R], I, T](implicit
    fitOperation: FitOperation[L],
    transformOperation: TransformDataSetOperation[L],
    predictorFitOperation: FitOperation[R]): FitOperation[ChainedPredictor[L, R]] = {
    new FitOperation[ChainedPredictor[L, R]] {
      override def fit(
  ...
  implicit def chainedEvaluationOperation[
      ...
      PredictionValue](
      implicit transformOperation: TransformDataSetOperation[T],
      evaluateOperation: EvaluateDataSetOperation[P],
      testingTypeInformation: TypeInformation[Testing],
      predictionValueTypeInformation: TypeInformation[PredictionValue])
    : EvaluateDataSetOperation[ChainedPredictor[T, P]] = {
    new EvaluateDataSetOperation[ChainedPredictor[T, P]] {
