...
public class RelBuilder {
  ...
  public RelBuilder matchRecognize(RexNode pattern, boolean strictStart, boolean strictEnd,
      Map<String, RexNode> patternDefinations, List<RexNode> measureList,
      RexNode after, Map<String, TreeSet<String>> subsets, boolean allRows,
      List<RexNode> partitionKeys, List<RexNode> orderKeys) {
    final List<RelFieldCollation> fieldCollations = new ArrayList<>();
    for (RexNode orderKey : orderKeys) {
      ...
      fieldCollations.add(new RelFieldCollation(((RexInputRef) orderKey).getIndex(), direction, nullDirection));
    }

    final RelDataTypeFactory.FieldInfoBuilder typeBuilder = cluster.getTypeFactory().builder();
    ...
    final RelNode match = matchFactory.createMatchRecognize(peek(), pattern,
      strictStart, strictEnd, patternDefinations, measures.build(), after, subsets, allRows,
      partitionKeys, RelCollations.of(fieldCollations), typeBuilder.build());
